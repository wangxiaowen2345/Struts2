...
<html>
  ...
  <script type="text/javascript">
  	...
  	function changepaytype(){
		...
		if(paytype==2)
			location.href="<%=path %>/admin/function/card.jsp?function=payreturned&value2=<s:property value="#cf.id"/>";
	...
	function pay(){
		...
		if(window.confirm('退货单号：<s:property value="#cf.number"/>，金额：<s:property value="#cf.reallytotal"/>元，请确认！')){
                 $.post("<%=path %>/admin.action?function=payreturned1",
					...
					function(data){
						...
						{
							alert("结款成功！");
							location.href="<%=path %>/admin.action?function=returnedlist1";
						}
